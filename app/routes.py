...
def index():
    if request.method == 'POST':
        ...
        meta_description = ""
        meta_match = re.search(r'<meta name=["\']description["\'] content=["\'](.+?)["\']\s*/?>', content, re.IGNORECASE)
        if meta_match:
        ...
        if not meta_description:
            ...
            if first_para:
                ...
                meta_description = cleaned_intro[:160] if cleaned_intro.lower().startswith(keyword.lower()) else f"{keyword}. {cleaned_intro}"[:160]
            else:
        ...
        # Meta
        if post_id:
            inject_meta_tags(post_id, meta_title=post_title, meta_description=meta_description, meta_keywords=keyword)
